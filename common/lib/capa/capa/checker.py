...
def main():
    ...
    for problem_file in args.files:
        log.info(u"Opening {0}".format(problem_file.name))

        try:
            problem = LoncapaProblem(problem_file, "fakeid", seed=args.seed, system=system)
        except Exception as ex:
            log.error(u"Could not parse file {0}".format(problem_file.name))
            log.exception(ex)
            continue
...
def command_test(problem):
    ...
    try:
        ...
        log_captured_output(sys.stdout,
                            u"captured stdout from {0}".format(problem))
        log_captured_output(sys.stderr,
                            u"captured stderr from {0}".format(problem))
...
def check_that_blanks_fail(problem):
    ...
    try:
        ...
        log.error(u"Blank accepted as correct answer in {0} for {1}"
                  .format(problem,
...
def check_that_suggested_answers_work(problem):
    ...
    * "a or d" is what's displayed, but only "a" or "d" is accepted, not the
    ...
    log.debug(u"Real answers: {0}".format(real_answers))
    if real_answers:
        try:
            ...
            log.error(u"The variable \"{0}\" specified in the ".format(uv_exc) +
                      u"solution isn't recognized (is it a units measure?).")
        except AssertionError:
            log.error(u"The following generated answers were not accepted for {0}:"
                      .format(problem))
            for question_id, result in sorted(real_results.items()):
                if result != 'correct':
                    log.error(u"  {0} = {1}".format(question_id, real_answers[question_id]))
        except Exception as ex:
            log.error(u"Uncaught error in {0}".format(problem))
            log.exception(ex)
...
def log_captured_output(output_stream, stream_name):
    ...
    if output_text:
        ...
        log.info(u"##### End {0} #####".format(stream_name))
