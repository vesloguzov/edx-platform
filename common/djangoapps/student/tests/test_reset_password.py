...
class ResetPasswordTests(EventTestMixin, CacheIsolationTestCase):
    ...
    @unittest.skipUnless(settings.ROOT_URLCONF == 'lms.urls', "Test only valid in LMS")
    @patch('django.core.mail.send_mail')
    @ddt.data(('crazy.awesome.com', 'Crazy Awesome Site'), ('edx.org', 'edX'))
    @ddt.unpack
    def test_reset_password_email_site(self, mktg_site_name, platform_name, send_email):
        """
        ...
        with patch("django.conf.settings.PLATFORM_NAME", platform_name):
            with patch("django.conf.settings.MKTG_SITE_NAME", mktg_site_name):
                req = self.request_factory.post(
                    '/password_reset/', {'email': self.user.email}
                )
                req.user = self.user
                ...
                reset_msg = reset_msg.format(mktg_site_name)
