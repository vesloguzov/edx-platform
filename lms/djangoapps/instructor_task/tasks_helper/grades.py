...
class CourseGradeReport(object):
    ...
    def _success_headers(self, context):
        ...
        """
        return (
            [_("Student ID"), _("Email"), _("Username"), _("Grade")] +
            self._grades_header(context) +
            ([_('Cohort Name')] if context.cohorts_enabled else []) +
            [u'{} ({})'.format(_('Experiment Group'), partition.name) for partition in context.course_experiments] +
            ([_('Team Name')] if context.teams_enabled else []) +
            [_('Enrollment Track'), _('Verification Status')] +
            [_('Certificate Eligible'), _('Certificate Delivered'), _('Certificate Type')] +
            [_('Enrollment Status')]
        )
    ...
    def _error_headers(self):
        ...
        """
        return [_("Student ID"), _("Username"), _("Error")]
...
class ProblemGradeReport(object):
    ...
    def generate(cls, _xmodule_instance_args, _entry_id, course_id, _task_input, action_name):
        ...
        header_row = OrderedDict([('id', _('Student ID')), ('email', _('Email')), ('username', _('Username'))])
        ...
        for assignment_type_name, subsection_infos in grading_context['all_graded_subsections_by_type'].iteritems():
            for subsection_index, subsection_info in enumerate(subsection_infos, start=1):
                for scorable_block in subsection_info['scored_descendants']:
                    ...
                    scorable_blocks_map[scorable_block.location] = [header_name + _(" (Earned)"),
                                                                    header_name + _(" (Possible)")]
