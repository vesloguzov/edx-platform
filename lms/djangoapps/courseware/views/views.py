...
from lms.djangoapps.required_student_data.decorators import require_student_data
...
class StaticCourseTabView(EdxFragmentView):
    ...
    @method_decorator(require_student_data)
...
def submission_history(request, course_id, location):
    ...
    if (student_identifier not in (request.user.username, request.user.email)) and (not staff_access):
