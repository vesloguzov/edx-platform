...
class CourseGrade(CourseGradeBase):
    ...
    def update(self):
        ...
        self.distinction = self._compute_distinction(grade_cutoffs, self.percent)
    ...
    @staticmethod
    def _compute_distinction(grade_cutoffs, percent):
        """
        Computes and returns distinction from percentage using grade cutoffs.
        """
        if len(grade_cutoffs) <= 1:
            return False

        return percent >= max(grade_cutoffs.values())
