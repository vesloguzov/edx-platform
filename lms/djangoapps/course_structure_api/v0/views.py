...
class CourseBlocksAndNavigation(ListAPIView):
    ...
    **Response Values**
        ...
        * blocks: A dictionary that maps block usage IDs to a collection of information about each block.
          ...
          * block_data: (dict) The JSON data for this block.
            Returned only if the "block_data" input parameter contains this block's type.
            ...
            if block_type in block_data_requested:
                if getattr(block, 'get_json', None):
                    block_value["data"] = block.get_json(
                        context=block_data_requested[block_type]
                    )
